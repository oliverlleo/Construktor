...
import { showSuccess, showError, showLoading, hideLoading, showInputDialog, showConfirmDialog } from './ui.js';
...
function setupWorkspaceSelector() {
    ...
    if (refreshSharedBtn) {
        refreshSharedBtn.addEventListener('click', async () => {
            ...
            try {
                await _loadSharedWorkspaces();
                showSuccess('Atualizado!', 'Recursos compartilhados atualizados com sucesso.');
            } catch (error) {
...
// Função removida - versão atualizada está mais abaixo no arquivo
...
async function createNewWorkspace() {
    ...
    if (result.confirmed && result.value) {
        ...
        try {
            ...
            hideLoading();
            showSuccess('Área de Trabalho Criada!', `"${result.value}" foi criada com sucesso.`);
        } catch (error) {
            hideLoading();
        ...
        const { value: formValues, isConfirmed } = await Swal.fire({
            ...
            preConfirm: () => {
                ...
                if (email.toLowerCase() === getUsuarioEmail()?.toLowerCase()) {
                    Swal.showValidationMessage('Você não pode compartilhar consigo mesmo.');
                    return false;
                }
...
async function sendWorkspaceInvitation(email, permission) {
    ...
    try {
        ...
        hideLoading();
        showSuccess('Convite enviado!', `Um convite para "${currentWorkspace.name}" foi enviado para ${email}.`);
    } catch (error) {
        hideLoading();
...
function updateSharedWorkspacesDisplay() {
    ...
    console.log("Atualizando exibição de workspaces compartilhados:", sharedWorkspaces.length);
    container.innerHTML = '';
    
    if (sharedWorkspaces.length === 0) {
        container.classList.add('hidden');
        emptyMessage.classList.remove('hidden');
        console.log("Nenhum workspace compartilhado encontrado, mostrando mensagem vazia");
    } else {
...
function renderSharedWorkspace(workspace, container) {
        ...
        <div class="shared-workspace-item bg-white rounded-lg border border-emerald-100 shadow-sm p-3 hover:shadow-md transition-shadow" 
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        <span class="font-medium text-slate-700 block text-sm">${workspace.name}</span>
                        <span class="text-xs text-slate-500">Por ${workspace.ownerName} • ${formatRoleText(workspace.role)}</span>
                    </div>
                </div>
    ...
    item.querySelector('.access-shared-workspace-btn').addEventListener('click', () => {
